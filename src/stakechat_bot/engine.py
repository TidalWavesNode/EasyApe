...
class Engine:
    ...
    # ── Command normalization (NEW: tolerant parsing) ──────────────────────────
    ...
    async def handle_text_async(
        ...
        # so minor punctuation (e.g. "balance -") doesn't break UX.
        cmd = self._normalize_cmd(text)

        # "start" UX (Telegram /start)
        if cmd in ("start",):
            return self._help()

        # global commands (work even if parser changes)
        if cmd in ("help", "h", "?"):
            return self._help()
        if cmd in ("privacy",):
        ...
        if cmd in ("quit", "exit", "stop"):
            # can't actually stop systemd from Telegram, but we can be explicit
            return BotResponse("🛑 EasyApe runs as a service. To stop it: `systemctl stop easyape`")

        # Stake/unstake still use the structured parser (amounts, netuid, etc)
    ...
    # ── Stake / Unstake flow (UNCHANGED from your working logic) ──────────────
    ...
    async def _balance(self) -> BotResponse:
        ...
        # Snapshot under free balance (restored + safe indentation)
        if total_cost:
    ...
    def _help(self) -> BotResponse:
        return BotResponse(
            ...
            "  `/start` / `start`\n"
            "  `help`\n"
            "  `quit` (shows how to stop service)\n\n"
            "*Examples*\n"
            "  `stake 0.5 31`\n"
            "  `unstake 0.25 31`\n"
